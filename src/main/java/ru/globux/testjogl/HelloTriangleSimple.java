...
import ru.globux.testjogl.framework.Semantic;
...
//            float[] model = FloatUtil.multMatrix(scale, rotateZ);
            Matrix4f scale = new Matrix4f().scale(0.5f, 0.5f, 0.5f, new Matrix4f(new float[16]));
            Matrix4f rotateZ = new Matrix4f(new float[16], 0)
                                    .rotate(diff, 0f, 0f, 1f, new Matrix4f(new float[16]));
            Matrix4f model = scale.mul(rotateZ);

            modelMatrixPointer.asFloatBuffer().put(model.get(FloatBuffer.allocate(16)));
        }
...
//        float[] ortho = FloatUtil.makeOrtho(new float[16], 0, false, -1f, 1f, -1f, 1f, 1f, -1f);
        Matrix4f ortho = new Matrix4f(new float[16], 0)
                            .setToOrtho( -1f, 1f, -1f, 1f, 1f, -1f);
        globalMatricesPointer.asFloatBuffer().put(ortho.get(FloatBuffer.allocate(16)));
